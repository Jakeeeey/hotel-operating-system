...
import { Plane, Waves, UtensilsCrossed, Sparkles, Wifi, Dumbbell } from "lucide-react"
...
export function HotelOverview(){
  ...
  const amenities: { icon: React.ReactNode; label: string }[] = [
    { icon: <Plane size={15} className="text-[#444444]" />, label: "Airport Shuttle" },
    { icon: <Waves size={15} className="text-[#444444]" />, label: "Private Beach" },
    {
      icon: <UtensilsCrossed size={15} className="text-[#444444]" />,
      label: "Fine Dining",
    },
    { icon: <Sparkles size={15} className="text-[#444444]" />, label: "Spa & Wellness" },
    { icon: <Wifi size={15} className="text-[#444444]" />, label: "Free WiFi" },
    { icon: <Dumbbell size={15} className="text-[#444444]" />, label: "Fitness Center" },
  ]
      ...
      <div className="grid grid-cols-[55%_45%] gap-10 items-start">
        ...
        <div>
          ...
          <p className="text-sm text-[#6b6b6b] leading-relaxed max-w-[520px] mb-8">
            Nestled between the turquoise lagoon and white-sand beaches, Azure Oasis
            offers an unrivaled escape from the everyday. Every detail is crafted for
            your comfort.
          </p>

...
export async function GET(request: Request) {
    try {
        ...
        let url = `${API_BASE_URL}/items/room_statuses?limit=-1&sort=-created_at`;
        if (search) {
            const filter = { status_name: { _icontains: search } };
...
export async function POST(request: Request) {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/items/room_statuses`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                ...(staticToken ? { 'Authorization': `Bearer ${staticToken}` } : {}),
